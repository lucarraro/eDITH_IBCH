...
if (!file.exists("support/Thur.rda")){
  ...
  # evaluate hydraulic variables (data from https://www.hydrodaten.admin.ch/en/seen-und-fluesse/messstationen-zustand)
  ...
  for (i in 1:length(hydroData$x)){
    ss <- rivnet::locate_site(hydroData$x[i], hydroData$y[i], Thur)
    siteAG[i] <- ss$AGnode
  }
  ...
  # evaluate landcover covariates
  # download the csv file called below from https://www.bfs.admin.ch/bfs/de/home/dienstleistungen/geostat/geodaten-bundesstatistik/boden-nutzung-bedeckung-eignung/arealstatistik-schweiz.assetdetail.20104753.html
  landCovCH <- read.csv("data/ag-b-00.03-37-area-csv.csv", sep = ";") # add this .csv file to the source folder
  ...
  save(Thur,file="support/Thur.rda",compress="xz")
} else {load("support/Thur.rda")}

# evaluate geographical covariates and create covariates data frame ####
...
covariates <- cbind(data.frame(logDrainageArea=log(Thur$AG$A), elevation=Thur$AG$Z, slope=Thur$AG$slope),
                    Thur$SC$locCov[c("urban","agriculture","forest")],
                    Thur$SC$upsCov[c("alluvial","alpine","molasses","moraines","peat")], 
                    geoCov)

# locate eDNA sampling sites ####
samplingSites <- read_excel("data/eDNASitesCoordinates.xlsx")
...
save(X_site,Y_site,site_key,sampSiteAG,file="support/eDNASamplingSitesInfo.rda")
...
for (ff in familyNames){
  ...
  if(!file.exists(paste0("results/",ff,".rda"))){
    ...
    out <- run_eDITH_BT(data, Thur, covariates, no.det=FALSE, ll.type = "nbinom",
                        mcmc.settings = list(iterations = 2.7e6, burnin=1.8e6, message = TRUE, thin = 10)) # short version
    out[["taxon"]] <- ff
    out <- eval_posterior_eDITH(out, Thur)#, covariates)

    pdf(paste0("results/",ff,"_map.pdf"),width=20/2.54,height=20/2.54)
    plot(out$probDetection_quantile, Thur, colLevels=c(0,1)); title(ff)
